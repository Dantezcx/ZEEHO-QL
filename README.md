...
- 多账号支持（环境变量用 `&` 或换行分隔）
- 社区任务接口（点赞 / 发帖 / 分享 / 删帖，可通过脚本顶部 `COMMUNITY_ENABLED` 开关配置）
...
脚本已实现社区四件套接口（点赞/发帖/分享/删除），用 [zeeho_sign_safe.js](zeeho_sign_safe.js#L226) 顶部的 `COMMUNITY_ENABLED` 按需开关：
...
const COMMUNITY_ENABLED = {
  ...
  post: true,   // 发帖
  share: true,  // 分享
  del: true,    // 删除（清理自己发的测试帖）
};
```

开启后脚本按「发帖 → 读列表取帖子 id → 点赞 → 分享 → 删除」顺序自动执行。

> 💡 **删帖问题已解决**：此前调用返回 `code=30121`，原因是查询参数被直接拼进 URL 路径、未参与签名计算，导致签名体为空。改为通过 `params` 传参（使 query 参与签名）后删除正常，返回 `code=10000`。
...
本脚本仅用于个人学习与自动化研究，接口与 ZEEHO 官方 App 无任何关联。请自行遵守 ZEEHO 服务条款及相关法律法规，并注意：
- 凭证必须自行通过正当渠道获取，本仓库不提供任何密钥
- 请勿将 `APP_SECRET` 等敏感凭证上传到任何公开渠道
- 请勿用于商业用途或批量注册/薅羊毛等违反服务条款的行为
- 请勿传播修改后的版本用于规避官方风控
